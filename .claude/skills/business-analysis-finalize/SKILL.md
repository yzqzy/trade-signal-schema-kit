...
5. 若用户要入站点：确认 `business_analysis_manifest.json` 的 `finalNarrativeStatus=complete`，执行 `pnpm run reports-site:emit -- --run-dir <business-analysis run>` → `pnpm run sync:reports-to-app`；正文发布为单篇 Markdown，不拼接 `qualitative_report.md` / `qualitative_d1_d6.md` 文件名标题。
